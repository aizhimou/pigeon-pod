...
- **👥 다중 사용자 및 역할 기반 액세스 제어**: 세분화된 권한으로 여러 계정을 관리하고 시스템 및 피드 구성을 관리자 사용자로 제한합니다.
...
- **🔔 다운로드 실패 요약 및 알림**: 자동 재시도가 모두 소진되면 이메일 또는 Webhook으로 실패 작업 요약을 받아볼 수 있습니다.
...
- **🔒 내장 SSL/TLS 암호화**: 암호화된 웹 및 RSS 트래픽에 대한 기본 SSL 지원으로 셀프 호스팅 인스턴스를 보호합니다.
- **📈 YouTube API 사용량 인사이트**: 동기화가 한도에 닿기 전에 할당량 사용량을 확인합니다.
- **🔄 OPML 구독 내보내기**: 구독을 쉽게 내보내 다른 팟캐스트 클라이언트로 옮길 수 있습니다.
- **⬆️ 앱 내 yt-dlp 관리**: 앱을 벗어나지 않고 런타임을 관리하고, 활성 버전을 전환하고, yt-dlp를 업데이트합니다.
