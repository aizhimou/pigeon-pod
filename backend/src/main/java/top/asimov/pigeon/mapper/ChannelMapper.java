...
public interface ChannelMapper extends BaseMapper<Channel> {
  ...
  @Select("SELECT c.id, c.handler, c.custom_title, c.title, c.cover_url, c.custom_cover_ext, c.description, c.source, c.audio_quality, c.last_updated_at, c.auto_download_enabled, " +
      "max(e.published_at) as last_published_at " +
      "FROM channel c LEFT JOIN episode e ON c.id = e.channel_id " +
      "GROUP BY c.id, c.handler, c.custom_title, c.title, c.cover_url, c.description, c.source, c.audio_quality, c.last_updated_at, c.auto_download_enabled " +
