...
- Simplified subscription flow: channels fetch a fixed 50 episodes (1 page), playlists fetch all episodes and persist them all; `autoDownloadLimit` controls the number of episodes to auto-download on first subscription (default 3) when `autoDownloadEnabled` is on, with remaining episodes saved as `READY` status (metadata only). Users can manually download additional episodes on-demand from the Feed detail page.
- Scheduled incremental sync: `ChannelSyncer`/`PlaylistSyncer` periodically refresh subscriptions, always fetching and storing new metadata; auto-downloads are only triggered when `autoDownloadEnabled` is on (limited by `autoDownloadLimit`). Historical episodes can be manually fetched via the "Fetch History Episodes" button on the Feed detail page.
...
- **Feed (abstract)**: `id`, YouTube `source`, default/custom titles and covers (`customTitle`, `customCoverExt`, derived `customCoverUrl`), keyword filters split between title and description, `minimumDuration`, `maximumDuration`, `autoDownloadEnabled`, `autoDownloadLimit`, `maximumEpisodes`, download knobs (`DownloadType`, `audioQuality`, `videoQuality`, `videoEncoding`), sync markers (`lastSyncVideoId`, `lastSyncTimestamp`), timestamps (`subscribedAt`, `lastUpdatedAt`). `FeedType` distinguishes channels vs playlists. Metadata sync is always enabled; only media downloads are gated by `autoDownloadEnabled`.
...
1. **Subscription onboarding**  
   ...
   - When the admin confirms, `FeedController.add` routes to `FeedService.add → FeedHandler.add → ChannelService/PlaylistService.saveFeed`. `AbstractFeedService` normalizes `autoDownloadLimit` (default 3 when `autoDownloadEnabled=true`): channels fetch a fixed 1 page (50 episodes), playlists fetch all episodes, and all episodes are persisted as `READY`. If `autoDownloadEnabled` is on, the first `autoDownloadLimit` episodes are marked as `PENDING` and trigger download; otherwise they remain `READY`. The `DownloadTaskEvent(INIT)` is emitted to background workers for async initialization.
2. **Incremental sync**  
   - `ChannelSyncer` and `PlaylistSyncer` run hourly, ask their services for `findDueForSync(now)` results (based on `lastSyncTimestamp` or `episodeSort` rules), then call `refreshChannel/refreshPlaylist`. The refresh method fetches new YouTube videos until it hits `lastSyncVideoId`, stores rows through `EpisodeService.saveEpisodes`, updates sync markers, and fires an `EpisodesCreatedEvent` only if `autoDownloadEnabled` is on. New episodes are persisted as `READY`; when auto-download is enabled, the first N episodes (limited by `autoDownloadLimit`) are marked `PENDING` and queued, the rest stay `READY`.  
...
3. **Download pipeline**  
   ...
   - `EpisodeCleaner` runs every 2 hours. It first uses grouped SQL aggregates to locate channels and playlists whose COMPLETED episode count exceeds their `maximumEpisodes`, then fetches only the older COMPLETED rows that need pruning, deletes their media/subtitle files from disk, and resets their `download_status` back to `READY` while keeping the Episode rows themselves so metadata remains browsable.
